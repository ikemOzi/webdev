...
class Ship:
    ...
    def move_lasers(self, vel, obj): # check if enemy laser collides with player
        self.cooldown()
        for laser in self.lasers:
            laser.move(vel)
...
class Player(Ship):
    def __init__(self, x, y, health=100):
        ...
        self.laser_img = YELLOW_SPACE_SHIP
        self.mask = pygame.mask.from_surface(self.ship_img) # helps with hitbox
...
def main():
    ...
    while run:
        ...
        for enemy in enemies[:]: # this creates a copy so when we are removing values it does not creat an issue
